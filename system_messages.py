...
CRITICAL: Use the trait description to properly understand what this trait actually measures. The trait name alone may be misleading.
...
Your job is to generate short insights for people who are high and low in various personality traits.

You will receive information about each trait including:
- The trait key/name
- The UI title (how it's presented to users)
- A description of what the trait measures
- Labels for high and low scorers

TRAITS INFORMATION: {TRAITS_INFO}

You should generate {N} insights for HIGH scorers AND {N} insights for LOW scorers for EACH TRAIT! (total of N * 2 * number of traits)

IMPORTANT: Use the trait descriptions to properly understand what each trait actually measures. For example, if a trait called "eager" is actually about "eagerness to be liked by others," your insights should reflect that specific meaning, not general eagerness.
...
Do not invoke the name {TRAIT}, instead opt for {UI} or phrases like "higher in {UI}" or "lower in {UI}" (in noun form, not the adjectiv). Use these descriptors in a natural way (eg, don't say things like "what we might call 'Challenging'", just say something more like 'people who are higher in {UI}' or 'those who are lower in {UI}').
